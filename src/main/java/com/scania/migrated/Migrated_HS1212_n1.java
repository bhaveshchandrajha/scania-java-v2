...
class WarrantyClaimService {
    ...
    private BigDecimal hersteller = BigDecimal.valueOf(1);
    private int tage = 14;

    public void processWarrantyClaim(String art, String g7100p, String g7101p, String g7102p, String g7103p, String g7104p) {
        if (art == null || art.isBlank()) {
            return;
        }

        Hsg71pf claim = findClaim(g7100p, g7101p, g7102p, g7103p, g7104p);
        if (claim == null) {
            return;
        }
        ...
        BigDecimal ganrn = parseDecimal(g7101p, 5, 0);
        Optional<S3f003> partnerOpt = s3f003Repository.findById(String.format("%05d", ganrn.intValue()));
        if (!partnerOpt.isPresent()) {
            return;
        }

        S3f003 partner = partnerOpt.get();
        BigDecimal matcos = calculateMaterialCost(partner, claim.getRepDatum());
        ...
        if ("6".equals(art) || "7".equals(art) || "8".equals(art)) {
            return;
        }

        if (claim.getStatusCodeSde() != null && claim.getStatusCodeSde().compareTo(BigDecimal.valueOf(20)) < 0) {
            validateClaimHeader(claim);
        }

        processClaimDetails(claim);
    }

    private Hsg71pf findClaim(String g7100p, String g7101p, String g7102p, String g7103p, String g7104p) {
        return hsg71pfRepository.findById(g7100p).orElse(null);
    }

    private void determineBereich(Hsg71pf claim) {
        if (claim.getBereich() != null && !claim.getBereich().isBlank()) {
            bereich = claim.getBereich();
        } else if (claim.getProduktTyp() != null && claim.getProduktTyp().compareTo(BigDecimal.valueOf(2)) == 0) {
            bereich = "3";
        } else if (claim.getProduktTyp() != null && claim.getProduktTyp().compareTo(BigDecimal.valueOf(3)) == 0) {
            bereich = "6";
        } else {
            bereich = "1";
        }
    ...
    private BigDecimal calculateMaterialCost(S3f003 partner, BigDecimal repairDate) {
    ...
    private void validateClaimHeader(Hsg71pf claim) {
        boolean hasError = false;

        if (claim.getProduktTyp() == null || 
            claim.getProduktTyp().compareTo(BigDecimal.ONE) < 0 || 
            claim.getProduktTyp().compareTo(BigDecimal.valueOf(5)) > 0) {
            hasError = true;
        }

        if (claim.getChassisNr() != null && !claim.getChassisNr().isBlank()) {
        ...
        if (claim.getRepDatum() != null) {
            if (!isValidDate(claim.getRepDatum())) {
                hasError = true;
            }
        } else {
            hasError = true;
        ...
        if (claim.getKmStand() != null && claim.getKmStand().compareTo(BigDecimal.ZERO) <= 0 && 
            claim.getChassisNr() != null && !claim.getChassisNr().isBlank()) {
            hasError = true;
        }
        ...
        if (hasError) {
            updateClaimStatus(claim, BigDecimal.ZERO);
        } else {
            updateClaimStatus(claim, BigDecimal.ONE);
        }
    }

    private boolean isValidDate(BigDecimal dateValue) {
        if (dateValue == null || dateValue.compareTo(BigDecimal.ZERO) <= 0) {
            return false;
        }
        
        String dateStr = String.format("%08d", dateValue.intValue());
    ...
    private void updateClaimStatus(Hsg71pf claim, BigDecimal status) {
        if (claim.getStatusCodeSde() == null || claim.getStatusCodeSde().compareTo(BigDecimal.valueOf(3)) < 0) {
            claim.setStatusCodeSde(status);
            hsg71pfRepository.save(claim);
        }
    }

    private void processClaimDetails(Hsg71pf claim) {
        // Process claim positions and details
    }

    private BigDecimal parseDecimal(String value, int precision, int scale) {
        try {
    ...
    private String formatDate(BigDecimal dateValue) {
        if (dateValue == null || dateValue.compareTo(BigDecimal.ZERO) == 0) {
            return "";
        }
    ...
    private LocalDate convertToLocalDate(BigDecimal dateValue) {
        if (dateValue == null || dateValue.compareTo(BigDecimal.ZERO) == 0) {
            return null;
        }
    ...
    private long calculateMonthsBetween(LocalDate date1, LocalDate date2) {
...
class ClaimProcessingContext {
    ...
    public ClaimProcessingContext(String dealerId, String claimNumber, String failureNumber) {
    ...
    public String getFailureNumber() { return failureNumber; }
    public void setFailureNumber(String failureNumber) { this.failureNumber = failureNumber; }
