...
public class WarrantyClaimService {
    ...
    public void processWarrantyClaim(String art, String g7100p, String g7101p, 
        ...
        } else if (claim.getProduktTyp().compareTo(BigDecimal.valueOf(2)) == 0) {
            bereich = "3";
        } else if (claim.getProduktTyp().compareTo(BigDecimal.valueOf(3)) == 0) {
            bereich = "6";
        } else {
            bereich = "1";
        }
    ...
    private boolean validateClaimHeader(Hsg71pf claim) {
        ...
        // Validate mileage is zero when no chassis number
        if (claim.getKmStand().compareTo(BigDecimal.ZERO) != 0 && 
            (claim.getChassisNr() == null || claim.getChassisNr().isBlank())) {
            valid = false;
        }
    ...
    private void createNewClaim(Hsg71pf claim) {
        // Logic for creating a new claim
        // Set initial status
        claim.setStatusCodeSde(BigDecimal.ONE);
    ...
    private void processAndValidateClaim(Hsg71pf claim) {
        // Validation and processing logic
        if (claim.getStatusCodeSde().compareTo(BigDecimal.valueOf(20)) < 0) {
            boolean headerValid = validateClaimHeader(claim);
            if (!headerValid) {
                return;
            }
        }
        ...
        // Update claim status
        if (claim.getStatusCodeSde().compareTo(BigDecimal.valueOf(3)) < 0) {
            if (claim.getAnzFehler().compareTo(BigDecimal.ZERO) == 0) {
    ...
    private void standardClaimProcessing(Hsg71pf claim) {
        // Standard claim processing workflow
        // Check if minimum claim
        if ("00000000".equals(claim.getClaimNrSde())) {
            // Process as minimum claim
            return;
        }
        
        // Process claim positions and validations
        processClaimPositions(claim);
        
        // Update claim
        hsg71pfRepository.save(claim);
    }
    
    private void processClaimPositions(Hsg71pf claim) {
        // Logic to process claim positions
        // This would involve reading from additional tables (HSG73PF, HSGPSPF, etc.)
        // and performing calculations and validations
    }
    ...
    private BigDecimal parseFormattedDate(String formattedDate) {
        if (formattedDate == null || formattedDate.isBlank()) {
            return BigDecimal.ZERO;
        }
        String[] parts = formattedDate.split("\\.");
        if (parts.length == 3) {
            return new BigDecimal(parts[2] + parts[1] + parts[0]);
        }
        return BigDecimal.ZERO;
    }
}
