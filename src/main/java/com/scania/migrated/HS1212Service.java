...
public class HS1212Service {
    ...
    public void processClaimManagement(String art, String g7100p, String g7101p, 
        ...
        if (art.equals("6") || art.equals("7") || art.equals("8")) {
            performHeaderChecks();
            return;
        }

        if (isStatusLessThan20()) {
            performHeaderChecks();
    ...
    private void loadClaimHeader() {
        ...
        loadPartnerData();
        calculateLabourValue(claim);
        readPositions(claim.getPakz(), claim.getClaimNr());
    }

    private void loadPartnerData() {
        BigDecimal partnerNo = ganrn != null ? ganrn : new BigDecimal(0);
        Optional<S3f003> partnerOpt = s3f003Repository.findByDistWrntyCustNo(
            String.format("%048d", partnerNo.longValue()));
    ...
    private void calculateMaterialCost(S3f003 partner) {
        BigDecimal repairDate = new BigDecimal(0);
        
        if (repairDate.compareTo(partner.getEffPerStartDate1()) >= 0 && 
            repairDate.compareTo(partner.getEffPerEndDate1()) <= 0) {
            matcos = partner.getStndLabourRtHr();
            matcos = matcos.multiply(partner.getDistLabUplift());
        } else if (repairDate.compareTo(partner.getEffPerStartDate2()) >= 0 && 
                   repairDate.compareTo(partner.getEffPerEndDate2()) <= 0) {
            matcos = partner.getLabourRtHr2();
            matcos = matcos.multiply(partner.getDistLabourUpliftFactor2());
        } else if (repairDate.compareTo(partner.getEffPerStartDate3()) >= 0 && 
                   repairDate.compareTo(partner.getEffPerEndDate3()) <= 0) {
            matcos = partner.getLabourRtHr3();
            matcos = matcos.multiply(partner.getDistLabourUpliftFactor3());
        }
    }

    private void calculateLabourValue(Hsg71pf claim) {
        // Calculate labour value based on repair date and partner data
    }
    ...
    private void performHeaderChecks() {
    ...
    private boolean isStatusLessThan20() {
        return false;
    }
    ...
    private boolean isMinimumClaim() {
        return false;
    }
