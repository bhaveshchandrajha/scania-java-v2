...
public class HS1212Service {
    ...
    public void processClaimManagement(String art, String g7100p, String g7101p, 
        ...
        if (art.equals("5")) {
            // Display mode
            return;
        } else if (art.equals("6") || art.equals("7")) {
            performHeaderCheck();
            performAction06();
            return;
        } else if (art.equals("8")) {
            art = "";
        }

        if (g71170 != null && g71170.compareTo(new BigDecimal(20)) < 0) {
            performHeaderCheck();
        ...
        if (g71160 != null && g71160.equals("00000000")) {
            displayHeaderScreen();
            return;
        }
    ...
    private void loadClaimHeader() {
        ...
        formatDates(claim);
        determineBereich(claim);
        checkMinimumClaim(claim);
        retrievePartnerNumber();
        calculateLabourValue();
        readPositions(claim.getPakz(), claim.getClaimNr());
        prepareClaimData(claim);
    }

    private void formatDates(Hsg71pfEntity claim) {
        // Format repair date and registration date
        if (claim.getRechDatum() != null) {
            g7102d = formatDate(claim.getRechDatum());
        }
        if (claim.getRepDatum() != null) {
            g7109d = formatDate(claim.getRepDatum().toString());
        }
    }
    ...
    private void determineBereich(Hsg71pfEntity claim) {
        if (claim.getBereich() != null && !claim.getBereich().isEmpty()) {
            bereich = claim.getBereich();
        } else if (claim.getProduktTyp() != null && claim.getProduktTyp().compareTo(new BigDecimal(2)) == 0) {
            bereich = "3";
        } else if (claim.getProduktTyp() != null && claim.getProduktTyp().compareTo(new BigDecimal(3)) == 0) {
            bereich = "6";
        } else {
            bereich = "1";
        }
    ...
    private void checkMinimumClaim(Hsg71pfEntity claim) {
        // Check if minimum claim is allowed
        if (claim.getClaimNrSde() == null || claim.getClaimNrSde().isEmpty()) {
            // Check for positions with status 30 or 16
        }
    }

    private void retrievePartnerNumber() {
        // Retrieve partner number from S3F003
        Optional<S3f003Entity> partnerOpt = s3f003Repository.findByDistWrntyCustNo(String.valueOf(ganrn));
        if (!partnerOpt.isPresent()) {
            return;
        }
        S3f003Entity partner = partnerOpt.get();
        calculateMaterialCost(partner);
    }

    private void calculateMaterialCost(S3f003Entity partner) {
        // Calculate material cost based on effective dates
        BigDecimal repairDate = g71090;
        if (repairDate != null) {
            if (isDateInRange(repairDate, partner.getEffPerStartDate1(), partner.getEffPerEndDate1())) {
                matcos = partner.getStndLabourRtHr().multiply(partner.getDistLabUplift());
            } else if (isDateInRange(repairDate, partner.getEffPerStartDate2(), partner.getEffPerEndDate2())) {
                matcos = partner.getLabourRtHr2().multiply(partner.getDistLabourUpliftFactor2());
            } else if (isDateInRange(repairDate, partner.getEffPerStartDate3(), partner.getEffPerEndDate3())) {
                matcos = partner.getLabourRtHr3().multiply(partner.getDistLabourUpliftFactor3());
            }
        }
    }

    private boolean isDateInRange(BigDecimal date, BigDecimal startDate, BigDecimal endDate) {
        return date.compareTo(startDate) >= 0 && date.compareTo(endDate) <= 0;
    }

    private void readPositions(String dealerId, String claimNo) {
        // Read positions for the claim
    }

    private void prepareClaimData(Hsg71pfEntity claim) {
    ...
    private void performHeaderCheck() {
        ...
        if (g7111a != null) {
            BigDecimal produktTyp = new BigDecimal(g7111a);
            if (produktTyp.compareTo(BigDecimal.ONE) < 0 || produktTyp.compareTo(new BigDecimal(5)) > 0) {
                in50 = true;
            }
        }

        if (g7106a != null && !g7106a.isEmpty()) {
            // Validate chassis number
        } else {
            // Check if claim type is 4
        }

        // Validate repair date
        if (g7109a != null && g7109a.compareTo(BigDecimal.ZERO) > 0) {
        ...
        // Validate mileage
        if (g7110a != null && g7110a.compareTo(BigDecimal.ZERO) <= 0 && g7106a != null && !g7106a.isEmpty()) {
            in53 = true;
        }
        ...
        updateClaimStatus(in50, in51, in52, in53, in54);
    }

    private void updateClaimStatus(boolean in50, boolean in51, boolean in52, boolean in53, boolean in54) {
    ...
    private void displayHeaderScreen() {
        // Display header screen for editing
    }

    private void determineStatus() {
        // Determine and display claim status
    }
    ...
    private void performAction06() {
        // Perform action 06 - create new claim entry
        loadClaimDefaults();
        determineFehlerNr();
        checkForCampaign();
        validateClaimType();
        searchForTextPositions();
        searchForCodes();
        displayClaimEntry();
    }

    private void loadClaimDefaults() {
        g7307a = "";
        g7308a = "";
        g7310a = "";
        g7311a = "";
        ...
        g7313a = "";
        g7314a = "";
        g7318a = BigDecimal.ZERO;
        g7319a = BigDecimal.ZERO;
        g7320a = BigDecimal.ZERO;
        g7324a = "1";
        g7325a = BigDecimal.ZERO;
        g7326a = BigDecimal.ZERO;
        g7327a = BigDecimal.ZERO;
        g7328a = "";
        g7332a = "";
        g7333a = "";
    }

    private void determineFehlerNr() {
        // Determine next available error number
        BigDecimal fnr = BigDecimal.ZERO;
        // Logic to find next available error number
        if (fnr.compareTo(new BigDecimal(9)) > 0) {
            // Ask to create new claim
        }
    }

    private void checkForCampaign() {
        // Check if campaign already exists
    }

    private void validateClaimType() {
        // Validate claim type based on chassis number
        if (g71060 == null || g71060.isEmpty()) {
            g7324a = "4";
        }
    }

    private void searchForTextPositions() {
        // Search for text positions in GPS
    }

    private void searchForCodes() {
        // Search for codes in AUFWSKF
    }

    private void displayClaimEntry() {
        // Display claim entry screen
    }

    private BigDecimal g71170;
    private String g71160;
    private String g7102d;
    private String g7109d;
    private BigDecimal g71090;
