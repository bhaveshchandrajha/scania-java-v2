...
public class HS1212Service {
    ...
    private BigDecimal maxkm = BigDecimal.ZERO;
    ...
    public void processClaimManagement(String art, String g7100p, String g7101p, 
        ...
        // Determine area based on product type
        if (claim.getBereich() != null && !claim.getBereich().trim().isEmpty()) {
            bereich = claim.getBereich();
        } else if (claim.getProduktTyp() != null) {
            if (claim.getProduktTyp().intValue() == 2) {
                bereich = "3";
        ...
        BigDecimal matcos = calculateMaterialCost(claim.getRepDatum(), partner);
        ...
        if (claim.getClaimNrSde() != null && "00000000".equals(claim.getClaimNrSde())) {
            // Display minimum claim screen
            return;
        }
        
        // Main processing loop would continue here
        // This represents the ANFANG tag and subsequent processing
    }
    
    private BigDecimal calculateMaterialCost(BigDecimal repairDate, S3f003 partner) {
        ...
        // Check date ranges and calculate cost
    ...
    private boolean validateClaimHeader(Hsg71pf claim) {
        ...
        if (claim.getChassisNr() != null && !claim.getChassisNr().trim().isEmpty()) {
            // Additional chassis validation would go here
        }
        ...
        // Validate registration date if mileage > 2000
        if (claim.getKmStand() != null && claim.getKmStand().intValue() > 2000) {
    ...
    private BigDecimal parseFormattedDate(String formattedDate) {
        ...
        if (parts.length == 3) {
            ...
            String day = parts[0];
            return new BigDecimal(year + month + day);
        }
